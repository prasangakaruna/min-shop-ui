...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => {
    ...
    apiRequest<{
      ...
      per_page: number;
      total: number;
    }>('/store/purchase-orders', { token, storeId: currentStore.id, query })
      .then((res) => {
        setOrders(res.data ?? []);
        setTotal(res.total ?? 0);
        setLastPage(res.last_page ?? 1);
      })
      .catch((e) => setError(e instanceof Error ? e.message : 'Failed to load purchase orders'))
    ...
    <div className="p-6 space-y-6 min-h-full">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 text-sm">Track inventory you order from suppliers.</p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 px-4">
            ...
            <p className="text-gray-500 text-sm mt-1">Create one to track incoming inventory.</p>
          </div>
        ) : (
                ...
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-medium">
                      <Link href={`/admin/orders/${order.id}`} className="text-mint hover:underline">
                        #{order.number}
                      </Link>
                    </td>
