...
export default function ProApiSettingsPage() {
  ...
  // Pro admin only: redirect if not pro_admin
  useEffect(() => {
    const userType = getCookie(USER_TYPE_COOKIE);
    if (userType && userType !== 'pro_admin') {
    ...
    return (
      <div className="flex min-h-full items-center justify-center p-6">
        <div className="w-full max-w-md rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-white p-8 text-center shadow-sm">
          ...
          <Link href="/admin/pro" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-mint px-4 py-2.5 text-sm font-medium text-white hover:opacity-90">
            ← Back to Pro & Admin
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-full bg-gray-50/60">
      <header className="border-b border-gray-200 bg-white">
        <div className="mx-auto max-w-6xl px-6 py-6">
          <Link href="/admin/pro" className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-500 transition hover:text-mint">
            ← Pro & Admin
          </Link>
          <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-gray-900">API & Webhooks</h1>
      ...
      <div className="mx-auto flex max-w-6xl gap-8 px-6 py-8">
        ...
        <aside className="hidden w-56 shrink-0 md:block">
          <div className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
            ...
            <nav className="mt-1 space-y-0.5 text-sm">
              <span className="flex w-full items-center gap-2.5 rounded-lg bg-mint/10 px-2.5 py-2 font-medium text-mint">
                <IconSliders className="h-5 w-5 text-mint" />
                API Settings
              </span>
              <Link
                href="/admin/pro/api-settings#webhooks"
                className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-gray-600 hover:bg-gray-50"
              >
                <IconWebhook className="h-5 w-5 text-gray-400" />
                Webhooks
              </Link>
            </nav>
          </div>
        ...
        <main className="min-w-0 flex-1">
          ...
          <div className="mb-6 text-sm text-gray-500">Settings &gt; API Integration</div>
                      ...
                      {keys.length === 0 ? (
                        ...
                        keys.map((k) => (
                          <tr key={k.id} className="border-b border-gray-50 last:border-0">
                            ...
                            <td className="px-6 py-4">
                              <span className="inline-flex items-center gap-2 rounded-full bg-gray-100 px-3 py-1 font-mono text-gray-700">
                                {maskKey(k)}
                                <button
                                  type="button"
